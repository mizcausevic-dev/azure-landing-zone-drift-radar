...
function usage(): string {
  return [
    "Usage: azure-landing-zone-drift <export.json> [options]",
    "",
    "Options:",
    "  --format <json|markdown|summary>  Output format (default: summary)",
    "  --now <iso>                        Override analysis time",
    "  --stale-drift-after-hours <n>     Drift SLA before stale flag (default: 24)",
...
function parseArgs(argv: string[]) {
  ...
  let staleDriftAfterHours: number | undefined;
  ...
  while (args.length > 0) {
    ...
    switch (flag) {
      ...
      case "--now":
        now = args.shift();
        break;
      case "--stale-drift-after-hours":
        staleDriftAfterHours = Number(args.shift());
        break;
      case "--fail-on-high":
        failOnHigh = true;
        break;
  ...
  return { file, format, now, staleDriftAfterHours, failOnHigh, out };
}
...
async function main() {
  const { file, format, now, staleDriftAfterHours, failOnHigh, out } = parseArgs(process.argv.slice(2));
  const input = JSON.parse(fs.readFileSync(file, "utf8")) as LandingZoneDriftExport;
  const report = analyze(input, { now, staleDriftAfterHours });
