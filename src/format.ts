...
export function toMarkdown(report: DriftReport): string {
  const lines: string[] = [];
  lines.push(report.ok ? "# Azure landing-zone drift posture ✅" : "# Azure landing-zone drift posture ❌");
  lines.push("");
  lines.push(`Generated: \`${report.generatedAt}\``);
  lines.push("");
  lines.push("## Coverage");
  lines.push("");
  lines.push(`- Zones: **${report.zones}**`);
  lines.push(`- Current baselines: **${report.currentBaselines}**`);
  lines.push(`- Drifts: **${report.drifts}**`);
  lines.push(`- Guardrail drifts: **${report.guardrailDrifts}**`);
  lines.push(`- Network drifts: **${report.networkDrifts}**`);
  lines.push(`- Identity drifts: **${report.identityDrifts}**`);
...
export function toSummary(report: DriftReport): string {
  ...
  return `${report.zones} zones · ${report.drifts} drifts · ${counts.high} high · ${counts.medium} medium (${report.ok ? "ok" : "fail"})`;
}
